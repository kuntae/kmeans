...
__kernel void update_map(__global int *partitioned, __global Point *data, __global Centroid_Data_t *tmp_centroid){
    ...
    //cast 2X2 array. 
    __global Point (*D)[DATA_N/CLASS_N/4] = (__global Point (*)[DATA_N/CLASS_N/4])data;
    __global int (*P)[DATA_N/CLASS_N/4] = (__global int (*)[DATA_N/CLASS_N/4])partitioned;
    __global Centroid_Data_t (*C)[CLASS_N] = (__global Centroid_Data_t (*)[CLASS_N])tmp_centroid;
    //local copy.    
    ...
    if(local_j == 0 && local_i < CLASS_N){
        ...
        C[group_id][local_i].point.x = p.x;
        C[group_id][local_i].point.y = p.y;
        C[group_id][local_i].cnt = tmp_count;
    }
}
...
__kernel void update_reduce(__global Centroid_Data_t *tmp_centroid, __global Centroid_Data_t *finally){
    ...
    //cast.
    __global Centroid_Data_t (*C)[CLASS_N] = (__global Centroid_Data_t (*)[CLASS_N])tmp_centroid;
    __global Centroid_Data_t (*F)[CLASS_N] = (__global Centroid_Data_t (*)[CLASS_N])finally;
    __local Centroid_Data_t mid[UPDATE_SIZE][CLASS_N];
    ...
    for(g = local_size >> 1; g >= 1; g = g >> 1){
        if(local_i < g && local_j < CLASS_N ){
            mid[local_i][local_j].point.x += mid[local_i + g][local_j].point.x;
            mid[local_i][local_j].point.y += mid[local_i + g][local_j].point.y;
            mid[local_i][local_j].cnt += mid[local_i + g][local_j].cnt;
        }
        barrier(CLK_LOCAL_MEM_FENCE);
    }
     
    if(local_i == 0 && local_j < CLASS_N){
        F[group_id][local_j].point.x = mid[local_i][local_j].point.x;
        F[group_id][local_j].point.y = mid[local_i][local_j].point.y;
        F[group_id][local_j].cnt = mid[local_i][local_j].cnt;
